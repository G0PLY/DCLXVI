...
int StrengthTbl[3] = { 35, 25, 20 };
int MagicTbl[3] = { 15, 15, 35 };
int DexterityTbl[3] = { 25, 35, 20 };
int VitalityTbl[3] = { 30, 30, 30 };
int ToBlkTbl[3] = { 30, 20, 10 };
char *ClassStrTblOld[3] = { "Warrior", "Rogue", "Sorceror" }; // unused
int MaxStats[3][4] = { { 250, 75, 96, 30 }, { 75, 96, 250, 30 }, { 75, 250, 96, 30 } };
int ExpLvlsTbl[100] =
{
  0,
	1000,
...
void __fastcall NextPlrLevel(int pnum)
{
	...
	if ( CalcStatDiff(pnum) < 3 ) {
		plr[pnum]._pStatPts = CalcStatDiff(pnum);
	} else {
		plr[pnum]._pStatPts += 3;
	}
...
void __fastcall AddPlrExperience(int pnum, int lvl, int exp)
{
	...
	if ( plr[pnum]._pExperience >= ExpLvlsTbl[98] ) {
		plr[pnum]._pLevel = 99;
		return;
	}
